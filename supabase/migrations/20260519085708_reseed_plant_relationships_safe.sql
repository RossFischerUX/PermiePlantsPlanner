-- Corrective reseed: apply CR-01/WR-03/WR-04/WR-05 fixes to the live remote DB.
-- Sequence: (1) set created_at NOT NULL, (2) clear existing rows, (3) re-insert
-- all 15 pairs via the safe conjunctive resolver, (4) drop temp function.
-- Exact resolve strings from 03-SEED-MANIFEST.md (verified 2026-05-19, 1455 rows).

-- WR-05: enforce NOT NULL on created_at (safe — all existing rows have non-null value).
ALTER TABLE plant_relationships ALTER COLUMN created_at SET NOT NULL;

-- Clear existing rows so re-seed is idempotent against the live table.
DELETE FROM plant_relationships;

-- Safe resolver: conjunctive equality (lower + btrim) — no ILIKE wildcards, no OR ambiguity.
